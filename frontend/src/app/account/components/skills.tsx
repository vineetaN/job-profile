...
const Skill: React.FC<AccountProps> = ({user , isYourAccount}) => {
      ...
      <Card className='shadow-lg border-2 overflow-hidden'>
        <div className="bg-blue-500 p-6 border-b">
          <div className="flex items-center gap-3 mb-4">
            <div className="h-10 w-10 rounded-lg bg-blue-100 dark:bg-blue-9 flex items-center justify-center">
...
{/**Add Skills Input */}
{
  isYourAccount && <div className="flex gap-3 flex-col sm:flex-row">
    ...
    <Button onClick={addSkillHandler} 
...
<CardContent className='p-6'>
  ...
  <div className='flex flex-wrap gap-3'>
    {user.skills.map((e,i)=>(
      <div className="group relative inline-flex items-center gap-2 border-2 rounded-full hover:shadow-sm duration-200 transition-all pl-4 pr-3 py-2"
      key={i}
      >
        ...
        {
          isYourAccount && <button onClick={() => removeSkillHandler(e)}
...
<Award size={32} className='opacity-40'/>
  ...
  <CardDescription className='text-base'>
    {
      isYourAccount ? "No Skill Added yet . Start Building YUour profile!"
      :
