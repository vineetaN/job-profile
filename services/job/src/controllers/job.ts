...
export const updateJob = TryCatch(async(req: AuthenticatedRequest,res) => 
{
  ...
  {
    throw new ErrorHandler(403 , "Forbidden : Your are not allowed.")
  }

  const [updatedJob] = await sql `
  UPDATE jobs SET title = ${title} , description = ${description} , salary = ${salary} , location = ${location} ,
  role = ${role},
  job_type = ${job_type},
...
export const getAllApplicationForJob = TryCatch(async(req:AuthenticatedRequest,res) => 
{
  ...
  {
    throw new ErrorHandler(403 , "Forbidden , you are not allowd to perform this task")
  }
